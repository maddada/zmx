...
@test "unchanged OSC 7 is not logged again after output or queries" {
  # The Ghostex fork redacts paths from its single zmx.log, so the upstream
  # assertion on the logged cwd cannot be expressed here. The de-duplication
  # itself (Daemon.setPwd returning early on an unchanged cwd) is merged.
  skip "fork redacts cwd from diagnostics"
