...
const Daemon = struct {
    ...
    pub fn handleInit(
        ...
        if (restore_visible_only and is_leader and self.has_pty_output and self.has_had_client) {
            // CDXC:ZmxMobileAttach 2026-05-23-12:58:
            // iPhone Ghostex attaches with a narrow PTY and must receive a viewport snapshot formatted for that exact grid.
            // Resize before the visible-only restore so the first phone screen is normal-width terminal content instead of desktop-width rows that wrap and leave banded blank regions after the client resizes.
        ...
        if (self.has_pty_output and self.has_had_client) {
            ...
            // CDXC:ZmxMobileAttach 2026-05-23-19:58:
            // iPhone attach clients cannot safely ingest a full scrollback replay before UIKit gets touch turns.
            // The --visible-only attach mode restores only the active viewport on reattach while preserving normal desktop zmx attach behavior for terminals that can handle full scrollback restoration.
            const maybe_term_output = if (restore_visible_only)
                util.serializeVisibleTerminalState(self.alloc, term)
            else
                util.serializeTerminalState(self.alloc, term);
            if (maybe_term_output) |term_output| {
